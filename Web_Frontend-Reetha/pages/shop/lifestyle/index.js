...
useEffect(() => {
  if (data.length > 0 && !loading) { 
    console.log("Data updated, length:", data.length, "Loading:", loading);
    ...
    if (pendingChatbotFilters) {
      ...
      console.log("Filter request ID:", pendingChatbotFilters.filterRequestId);
      
      // First complete other data processing
      fetchLocations(data);
      createPriceFilters(data);
      processDiscountData(data);
      
      // Use a longer timeout to ensure all state updates and data processing is complete
      setTimeout(() => {
        console.log("Executing pending chatbot filter application...");
        
        // Verify data is still available before applying filters
        if (data.length > 0) {
          applyChatbotFiltersToData(pendingChatbotFilters.filterData, pendingChatbotFilters.originalPrompt);
        } else {
          console.warn("Data became empty, cannot apply pending filters");
        }
      ...
      console.log("No pending chatbot filters, applying sort to new data");
  ...
  const handleClearAllFilters = async () => {
    console.log("Clearing all filters, activeChatbotFilters:", activeChatbotFilters);
    ...
    // Clear any pending chatbot filters
    setPendingChatbotFilters(null);

    // Apply current sort to the original data only if no active chatbot filters
    if (!activeChatbotFilters) {
      console.log("No active chatbot filters, applying sort to original data");
      const sortedData = applySortToData(data);
      setFilteredData(sortedData);
    } else {
      console.log("Active chatbot filters present, keeping current filtered data");
    }
  };
...
const applySortToData = (dataToSort, sortMethod) => {
  ...
  const isProductFree = (product) => {
    ...
    return price === 0 && product.provider === "aahaas";
  };
  ...
  switch (effectiveSortMethod) {
    ...
    case 'priceHL':
      // Sort all products by price (high to low), free products will naturally come last
      sortedData = sortedData.sort((a, b) => {
        const priceA = getSafePrice(a);
        const priceB = getSafePrice(b);
...
const performAsyncSort = (dataToSort, sortMethod) => {
  ...
  useEffect(() => {
    if (filteredData.length > 0 && !isSorting) {
      handleSortChange();
    }
  }, [selectedSortMethod]);
  ...
  const applyChatbotFiltersToData = (filterData, originalPrompt) => {
    ...
    try {
      // Use the latest data state to ensure we're working with fresh data
      const currentData = data;
      console.log("Working with current data length:", currentData.length);
      ...
      // Apply chatbot filters to the data - start with the current data
      let filteredResults = [...currentData]; // Shallow copy for better performance
      ...
      if (filterData?.primaryKeywords && filterData.primaryKeywords.length > 0) {
        ...
        filteredResults = filteredResults.filter((item) => {
          ...
          // Check if item matches any of the primary keywords with exact word matching
          ...
          filteredResults = filteredResults.filter((item) => {
            ...
            if (item.currency && item.currency !== baseCurrencyValue?.base) {
              try {
                ...
                console.warn("Currency conversion failed for item:", item.lifestyle_id, conversionError);
                convertedPrice = productPrice; // Fallback to original price
              }
            }
      ...
      // Set active chatbot filters for display panel - use functional update
      setActiveChatbotFilters(() => ({
        filterData: JSON.parse(JSON.stringify(filterData)), // Deep copy to prevent reference issues
        originalPrompt,
      ...
      console.log(`Chatbot filter applied successfully: ${filteredResults.length} products found from ${currentData.length} total products`);
      
      // Show success message
      ToastMessage({
        status: "success",
        message: `AI filters applied - Found ${filteredResults.length} products`
      });
  ...
  const handleChatbotFilterData = async (filterData, originalPrompt) => {
    ...
    try {
      // Create a unique timestamp to track this filter request
      const filterRequestId = Date.now();
      console.log("Filter request ID:", filterRequestId);

      // Reset current filters before applying chatbot filters - use functional updates to ensure latest state
      ...
      if (filterData?.location && typeof filterData.location === 'object' && 
        ...
        console.log("Location comparison:", {
          ...
          shouldReload: shouldReloadProducts
        });
      }
      ...
      if (shouldReloadProducts && newLocationContext) {
        ...
        // Set pending filters with request ID for tracking
        console.log("Setting pending chatbot filters with ID:", filterRequestId);
      ...
      // Wait a brief moment to ensure all state updates are complete
      setTimeout(() => {
        console.log("Applying chatbot filters immediately (no location change)");
  ...
  <GooglePlacesAutocomplete
    apiKey={groupApiCode}
    selectProps={{
      value: googleLocation === 'search' ? null : googleLocation,
      ...
      styles: {
        container: (provided) => ({
          ...
          width: '100%',
          position: 'relative',
          zIndex: 9999  // High z-index for container
        }),
        control: (provided) => ({
          ...provided,
          paddingRight: '30px',
          backgroundColor: '#fff',
          zIndex: 9999  // High z-index for control
        }),
        menu: (provided) => ({
          ...provided,
          zIndex: 99999,  // Very high z-index for dropdown menu
          position: 'absolute',
          marginTop: '0px',
          width: '100%',
  ...
  {googleLocation !== 'search' && !isUsingGlobalLocation && (
    <div
      style={{
        ...
        cursor: 'pointer',
        zIndex: 10000,  // Higher than the input
        background: '#fff',
        borderRadius: '50%',
        padding: '2px',
        display: 'flex',
    ...
    >
      <CloseIcon sx={{ fontSize: '18px', color: '#dc3545' }} />
                            ...
                            <h5
                                                    ...
                                                    <div className="collection-collapse-block-content" style={{ position: 'relative', zIndex: 12 }}>
        ...
        <div style={{ padding: '8px' }}>
          ...
          />
              ...
              <LifestyleChatbot
                ...
                clearTrigger={clearAIFiltersTrigger}
              /> 
            </Container>
          </div>
