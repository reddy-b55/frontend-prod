...
export async function getServerSideProps(context) {
    ...
    const productUrl = `https://www.aahaas.com/product-details/nonessential/${name}?pID=${id}`;
...
const NonEssentialProductView = ({ dataset, pathId = dataset?.productId, canonicalURL = dataset?.canonicalURL }) => {
    ...
    const getprodviewdataByid = async () => {
        ...
        if (result) {
            ...
            if (generateSlug(result.discount_data[0].listing_title) !== router.query.name) {
                window.location.assign(`/product-details/nonessential/${generateSlug(result.discount_data[0].listing_title)}?pID=${router.query.pID}`)
            } else {
    ...
    const handleAddToCartMain = () => {
        if (isSelectData === false) {
            ...
            ToastMessage({ status: "error", message: "Please wait while we calculate shipping charges" })
        } else {
            ...
            const dataD = {
                ...
                discountMrp: Number(EssentialDetailPricing(productPrice, quantity, productData)['priceWithDiscount']) + Number(deliveryData.deliverCharge),
                netAmount: Number(EssentialDetailPricing(productPrice, quantity, productData)['priceWithDiscount']),
    ...
    const handleSelectAddressType = (e) => {
        ...
        setIsShippingCalculated(false); // Reset when location type changes
    ...
    // const getLocationDetils = (latitude, longitude) => {
    //     let city = '';
    //     try {
    //         geocodeByLatLng({ lat: Number(latitude), lng: Number(longitude) }).then(results => {
            ...
            await loadGoogleMapsAPI().then(() => {
                ...
                geocoder.geocode({ location: latLng }, (results, status) => {
                    if (status === "OK" && results[0]) {
                        ...
                        console.log(latitude + "," + longitude, city, productData, "Delivery response")
                        getTotalDistance(latitude + "," + longitude, city, productData);
                        setCustomerCustomAddress(results[0]['formatted_address']);
                        setCanDelivery(true)
                    } else {
    ...
    const getTotalDistance = async (latLonMain, city, product) => {
        setStartCalculatingPrice(true);
        setIsShippingCalculated(false); // Reset when starting calculation
        ...
        await axios.post('getDeliveryDetails', data, {
            ...
            if (response.data.status === 200) {
                setDeliveryData({
                    ...
                    deliverCharge: response.data.deliveryCharge,
                    deliveryRateID: response.data.deliveryRateID
                })
                setCanDelivery(true);
                setIsShippingCalculated(true); // Set to true when calculation is complete
                ...
                setCanDelivery(false);
                setIsShippingCalculated(true); // Still set to true as calculation is complete
            } else {
                setDeliveryData({
                    ...deliveryData,
                    status: '500',
            ...
            setCanDelivery(false);
            setIsShippingCalculated(true); // Set to true even on error
            setStartCalculatingPrice(false);
        })
        setStartCalculatingPrice(false);
    }
        ...
        if (chatCreating) {
            ...
            if (userStatus.userLoggesIn) {
                await addDoc(collection(db, "customer-chat-lists"), {
                    ...
                    comments: { category: 2, product_id: productData?.id },
    ...
    const onChangeDate = (value) => {
        ...
        setuserSelectedDate(moment(value).format('YYYY-MM-DD'))
    ...
    const handleSaveAddress = async () => {
        ...
        setCanDelivery(false)
        setLocationTypeId('')
        setLocationType(router?.query?.locationtyp || locationType || '')
        setSelectedLocationId('')
    ...
    useEffect(() => {
        if (router.query?.viewStatus === "update" && isSelectData === false && new Date(router.query?.prefereedDate)) {
            // console.log("date1333333333333333",router.query?.viewStatus === "update" && isSelectData === false ? new Date(router.query?.prefereedDate) : isSelectData ?userSelectedDate : null)
            setIsSelectData(true)
        }
        if (!loading) {
            updateInitialValue()
    ...
    return (
        <>
            ...
            <Head>
                ...
                <title>Aahaas - {dataset.productData.discount_data[0].listing_title} | Unique Non-Essential Goods with Aahaas</title>
                <meta name="description" content={`Explore ${dataset.productData.discount_data[0].listing_description} at Aahaas. Indulge in a curated selection of non-essential commodities that enhance your lifestyle. Shop now to find unique items that bring joy and inspiration to your everyday life.`} />
                {/* PDP Page */}
                <script
                        ...
                        __html: JSON.stringify({
                            ...
                            "itemListElement": [
                                ...
                                },
                                {
                                    ...
                                    "position": 2,
                                    "name": "Non-Essnetials",
                                    "item": "https://www.aahaas.com/product-details/nonessential/"
                                },
                                {
                                    "@type": "ListItem",
                                    "position": 3,
                                    "name": "Non-Essnetials",
                                    "item": `https://www.aahaas.com/product-details/nonessential/${generateSlug(dataset.productData.discount_data[0].listing_title)}?pID=${pathId}`
                                }
                            ]
                        }),
                    }}
            ...
            <CommonLayout parent="Home" title={'non-essential'} subTitle={'product view'} openSubFilter={() => openSubFilter()} showSearchIcon={false} showMenuIcon={true} location={false} subTitleParentLink={"/shop/nonessential"}>
                                                ...
                                                <Col lg="5" className="product-thumbnail p-0 m-0">
                                                    <Slider {...products} asNavFor={nav2} ref={(slider) => setSlider1(slider)} className="product-slick">
                                                        {pictures.split(',').map((vari, index) => (
                                                            <div key={index}>
                                                                <Media alt='product images' src={`${vari}`} className="img-fluid product-main-image px-4" style={{ width: "98%", height: "465px", objectFit: 'cover' }} loading="lazy" />
                                                            </div>
                                                        ))}
                                                    </Slider>
                                                    {/* <div className='d-flex overflow-hidden w-100 m-0 p-0 gap-2 justify-content-center mt-lg-3'>
                                                        {pictures.split(',').length > 1 && pictures.split(',').map((vari, index) => (
                                                            <Media alt='product other images' key={index} src={`${vari}`} style={{ minHeight: '80px', maxHeight: '80px', minWidth: '80px', maxWidth: '80px', objectFit: 'cover' }} loading="lazy" />
                                                        ))}
                                                    </div> */}
  ...
  router.push(
    `/vendor-products/${productData.seller_id}?vendorName=${encodeURIComponent(vendorName)}`
  );
}}
        ...
        >
          <StoreIcon style={{ fontSize: 16, marginRight: 5 }} />
                                                        ...
                                                        <div className="col-6 product-variant-head " style={{ width: '48%' }}>
                                                            ...
                                                            <DatePicker minDate={new Date(new Date().setDate(new Date().getDate() + Number(productData.time_to_delivery)))} onChange={onChangeDate} maxDate={new Date(productData.active_end_date)}
                                                                value={router.query?.viewStatus === "update" && isSelectData === false ? new Date(router.query?.prefereedDate) : isSelectData ? userSelectedDate : null} clearIcon={false} className='form-control py-2' />
                                                        </div>
...

                                                        {
                                                            ...
                                                            <div className="product-variant-head mt-2 col-12">
                                                                ...
                                                                {/* <Select options={shippingAddresses} onChange={handleChooseSavedLocation} value={shippingAddresses.find(data => data.key === selectedSavedLocation)} className="react__Select" /> */}
                                                            </div>
                                                        }
                                                                    ...
                                                                    <h5 className="m-0 p-0">
                                                                        <LocalShippingIcon style={{ fontSize: 16, marginRight: 5 }} />
                                                                        {
                                                                            CurrencyConverter(
                                                                                ...
                                                                                (isNaN(Number(CurrencyConverterOnlyRateWithoutDecimal(deliveryData.currency, deliveryData.deliverCharge, baseCurrencyValue)))) ||
                                                                                    !isFinite(Number(CurrencyConverterOnlyRateWithoutDecimal(deliveryData.currency, deliveryData.deliverCharge, baseCurrencyValue)))
                                                                                    ? 0
                                                                                    : CurrencyConverterOnlyRateWithoutDecimal(deliveryData.currency, deliveryData.deliverCharge, baseCurrencyValue),
                                                                                baseCurrencyValue
                                                                            )
                                                                        }
                                                                    </h5>
                                                                    <h3 className="m-0 p-0">
  {
    CurrencyConverter(
      baseCurrencyValue.base,
      (
        (Number(CurrencyConverterOnlyRateWithoutDecimal(productData.currency, productPrice.mrp, baseCurrencyValue)) * quantity) +
        (isNaN(Number(CurrencyConverterOnlyRateWithoutDecimal(deliveryData.currency, deliveryData.deliverCharge, baseCurrencyValue))) ||
          !isFinite(Number(CurrencyConverterOnlyRateWithoutDecimal(deliveryData.currency, deliveryData.deliverCharge, baseCurrencyValue)))
          ? 0
          : Number(CurrencyConverterOnlyRateWithoutDecimal(deliveryData.currency, deliveryData.deliverCharge, baseCurrencyValue)))
      ),
      baseCurrencyValue
    )
  }
                                                            ...
                                                            <div className='d-flex flex-row align-items-center col-12 gap-3 justify-content-end'>
                                                                <Button
                                                                    ...
                                                                    style={{
                                                                        ...
                                                                        padding: '10px 15px',
                                                                        borderRadius: '4px',
                                                                        opacity: (!isShippingCalculated || !deliveryData.status || !canDelivery) ? 0.6 : 1,
                                                                        cursor: (!isShippingCalculated || !deliveryData.status || !canDelivery) ? 'not-allowed' : 'pointer'
                                                                    }}
                                                                    onClick={handleAddToCartMain}
                                                                    disabled={!isShippingCalculated || !deliveryData.status || !canDelivery}
                                                                >
                                                                    {cartStatus.status === 'update' ? 'Update cart' : 'Add to cart'}
                                                                </Button>
                                                                <Button className="btn btn-sm btn-solid" style={{ fontSize: 12, padding: '10px 15px', borderRadius: '4px' }} onClick={handleChatInitiate}>
...
export default NonEssentialProductView;
