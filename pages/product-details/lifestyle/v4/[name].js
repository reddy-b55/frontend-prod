...
import { getCebAttractionDetails, getTicketAvailability } from "../../../../AxiosCalls/LifestyleServices/cebuService";
...
export async function getServerSideProps(context) {
  ...
  let response = []
  await getCebAttractionDetails(id).then((res) => {
    if (res === 'Something went wrong !' || res === '(Internal Server Error)') {
      ...
      console.log("res is cebuuuuuuuuuuuuuuuuuu 64", res)
      response = res
    }
  });
...
const LifestyleProdcutView = ({ dataset, pathId = dataset?.productId, canonicalURL = dataset?.canonicalURL }) => {
  ...
  console.log("Data set product isxxxx", dataset)
  const router = useRouter();
  let adultcount = router.query.lifeStyle_adult_count;
  let childcount = router.query.lifestyle_children_count;
    ...
    if (travelBuddiesStatus) {
      ...
      if (viewStatus === 'update') {
        dataD = {
          ...
          travel_buddy_adult_id: adultDetails.map((adult) => adult.id).join(","),
        ...
        dataD = {
          ...
          selectedTickets: selectedTickets || [],
          tickets: ticketDataPass || [],
          total_amount: ticketDataPass?.[0]?.price,
          transaction_amount: ticketDataPass?.[0]?.price,
          transaction_currency: "SGD",
          travel_buddy_adult_id: adultDetails.map((adult) => adult.id).join(","),
          travel_buddy_child_id: "",
          user_id: baseUserId.cxid,
  ...
  const loadProductData = async () => {
    ...
    console.log("response isssssssssss", response)
    if (response.success === true) {

      let lifeStyleInventoryRates = response?.lifeStyleInventoryRates;

      // if (generateSlug(response.data.name) !== router.query.name) {
      //   window.location.assign(`/product-details/lifestyle/v4/${generateSlug(response.data.name)}?pID=${router.query.pID}`)
      // }
    ...
    if (chatCreating) {
      ...
      if (userStatus.userLoggesIn) {
        ...
        await addDoc(collection(db, "customer-chat-lists"), {
          ...
          chat_category: 3,
          chat_related_id: 3,
          chat_avatar: lifeStyleData.images?.full,
          supplier_id: null,
          chat_name: lifeStyleData.title,
          comments: { category: 3, product_id: lifeStyleData?.sku_id },
  ...
  //   function countTicketsByCategory(ticketsData) {
  //     let childCount = 0;
  //     let adultCount = 0;  
  //     ticketsData.forEach(ticket => {  
  //         const ticketType = ticket.ticketDataSet.type;
  //         const quantity = ticket.quantity;

  //         const categoryMatch = ticketType.match(/\[([^\]]+)\]/);
  //         if (categoryMatch) {
  ...
  //             if (category === 'CHILD') {
  //                 childCount += quantity;
  //             } else if (category === 'SENIOR' || category === 'ADULT') {
  //                 adultCount += quantity;
  //             }
  //         } else {
  //             console.log("  No category found in brackets");
  //         }
  //         console.log("---");
  //     });
  //     adultcount = adultCount;
  ...
  function countTicketsByCategory(ticketsData) {
    let childCount = 0;
    let adultCount = 0; // This will include both adult and senior tickets

    ticketsData.forEach(ticket => {
      const ticketType = ticket.ticketDataSet.type;
      const quantity = ticket.quantity;

      // Extract the category from the type string (what's inside the brackets)
      const categoryMatch = ticketType.match(/\[([^\]]+)\]/);
      ...
      if (categoryMatch) {
        ...
        if (category === 'CHILD') {
          childCount += quantity;
        } else if (category === 'SENIOR' || category === 'ADULT' || category === 'SENOIR') {
          // Count SENIOR, ADULT, and SENOIR (typo) as adult tickets
          adultCount += quantity;
        }
      }
    });
    ...
    return {
      child: childCount,
      adult: adultCount, // This includes both adult and senior tickets
      total: childCount + adultCount
    };
  }
  ...
  const handleTicketSelectionContinue = (ticketDataD, tickets) => {
    ...
    console.log("Selected tickets main 12333:", ticketDataD);
    const ticketsType = countTicketsByCategory(tickets)
    setCustomerData({
      ...customerData,
      childCount: ticketsType.child,
      adultCount: ticketsType.adult,
    })
    // console.log("Selected tickets main 12333:", adultcount, childcount);
    ...
    const transformedTickets = tickets.map(ticket => ({
      ...
      price: ticketDataD?.total_amount || 0,
  ...
  const returnTotalAmount = () => {
    console.log("Selected tickets:xxxxxxxxxxxx", ticketDataPass);
    if (selectedTickets.length > 0) {
      const total = getTotalPrice();
      console.log("Total amount:", baseCurrencyValue);
      const currency = selectedTickets[0]?.ticketDataSet?.currency ||
        baseCurrencyValue?.base ||
        "USD";
      return CurrencyConverter(currency, ticketDataPass?.[0]?.price, baseCurrencyValue);
      return {
        total: total,
        currency: currency,
  ...
  return (
    <>
      ...
      <Head>
        ...
        <title>Aahaas - {dataset.productData?.data?.title} | Unique Experiences with Aahaas in Sri Lanka</title>
        <meta name="description" content={`Discover ${dataset.productData?.data?.title} and other fun activities in Sri Lanka! Explore recreational experiences, top tourist activities, and unforgettable adventures on Aahaas.`} />
        <script
            ...
            __html: JSON.stringify({
              ...
              "itemListElement": [
                ...
                },
                {
                  ...
                  "position": 3,
                  "name": "Lifestyle",
                  "item": `https://www.aahaas.com/product-details/lifestyle/v4/${generateSlug(dataset.productData?.data?.title)}?pID=${pathId}`
                }
              ]
            }),
          }}
    ...
    <li>
      <button onClick={handleBack} style={{ border: 'none', background: 'none' }}>
            ...
            <Row>
              ...
              <Col lg="9" sm="12" xs="12">
                    ...
                    />
                        ...
                        <Col lg="7" className="rtl-text p-0 m-0 px-2 px-lg-4 pe-0">
                          ...
                          <div className='product-right d-flex flex-wrap justify-content-between'>
                            ...
                            <h1 className="col-12 text-start mb-1 product-name-main" style={{ textTransform: 'uppercase', fontWeight: '500' }}>{lifeStyleData.title} </h1>
                            ...
                            <div className="col-12">
                              ...
                              <div className="form-control d-flex align-items-center mb-2 product-variant-head" style={{ padding: '10px 10px', fontSize: '14px' }} onClick={() => handleTicket(lifeStyleData?.id)}>
                                {ticketSelectionPage ? `Selected ${getTotalTickets()} ticket(s)` : "Select Ticket"}
...

                            {
                              ticketSelectionPage ?
                                <><div className="d-flex flex-column m-0 p-0 align-items-end justify-content-center mt-3 col-12">
                                  <h3 className="m-0 p-0">  {returnTotalAmount()}</h3>
                                  {ticketSelectionPage ? `Selected total ${getTotalTickets()} tickets` : null}
                                  <h1></h1>
                                </div>
                ...
                </Container>
                {
                  ...
                  <ProductTab
                    type='lifestyle'
                    height={lifeStyleData.images.full ? '355px' : packageStatus ? '380px' : '430px'}
                    showDesc={true} name={lifeStyleData.title} desc={lifeStyleData.description} showTermsndConditions={true} showndConditions={lifeStyleData?.tnc} />
                }
              </Col>
            </Row>
          </Container>
        ...
        <ModalBase isOpen={handlePopupStatus.customerDetails} toggle={handleCloseAllPopup} title="Choose Your Buddies" size='md'>
          <BuddyModal adultCount={customerData.adultCount} childCount={customerData.childCount} adultDetails={adultDetails} childsDetails={childsDetails} childAges={customerData.childAges} handleTravelBuddies={handleTravelBuddies} providerlife={"cebu"}></BuddyModal>
        </ModalBase>

        <ModalBase isOpen={handlePopupStatus.addToCart} toggle={handleCloseAllPopup} title="Select Your Cart" size='md'>
          <CartContainer handlePassData={handlePassData} productData={cartData} cartCategory={"Lifestyle"} provider={"cebu"} preId={preId || null} pageState={viewStatus === 'update' ? "CartEdit" : null} />
        </ModalBase>

        <ModalBase isOpen={showTicket} toggle={handleTicketModel} title="Select Ticket" size='lg'>
          <TicketSelector attractionTickets={lifeStyleData?.tickets} selectedDate={selectedDate} handleOnPressContinueData={handleTicketSelectionContinue} />
        </ModalBase>
